...
describe 'SelectList' do
  ...
  describe '#select' do
    context 'when finding by default' do
      ...
      it 'selects multiple options with Array' do
        browser.select_list(name: 'new_user_languages').clear
      ...
      it 'selects multiple options with multiple parameters' do
        browser.select_list(name: 'new_user_languages').clear
    ...
    context 'when finding by value' do
      ...
      it 'selects an option with a Regexp' do
        browser.select_list(name: 'new_user_languages').clear
        browser.select_list(name: 'new_user_languages').select(value: /1|3/)
        expect(browser.select_list(name: 'new_user_languages').selected_options.map(&:text)).to eq %w[Danish NO]
      end

      it 'selects multiple options with Array' do
        browser.select_list(name: 'new_user_languages').clear
        browser.select_list(name: 'new_user_languages').select(value: %w[1 3])
        list = %w[Danish NO]
        expect(browser.select_list(name: 'new_user_languages').selected_options.map(&:text)).to eq list
      end
    end
    ...
    context 'when finding by text' do
      it 'selects an option with a String' do
        browser.select_list(name: 'new_user_country').select(text: 'Denmark')
        expect(browser.select_list(name: 'new_user_country').selected_options.map(&:text)).to eq ['Denmark']
      end
      ...
      it 'selects multiple options with Array' do
        browser.select_list(name: 'new_user_languages').clear
        browser.select_list(name: 'new_user_languages').select(text: [/ish/])
        list = %w[Danish EN Swedish]
        expect(browser.select_list(name: 'new_user_languages').selected_options.map(&:text)).to eq list
      end
    end
    ...
    context 'when finding by label' do
      it 'selects an option with a String' do
        browser.select_list(name: 'new_user_languages').clear
        browser.select_list(name: 'new_user_languages').select(label: 'NO')
        expect(browser.select_list(name: 'new_user_languages').selected_options.map(&:text)).to eq ['NO']
      end

      it 'selects an option with a Regexp' do
        browser.select_list(name: 'new_user_languages').clear
        browser.select_list(name: 'new_user_languages').select(label: /^N/)
        expect(browser.select_list(name: 'new_user_languages').selected_options.map(&:text)).to eq ['NO']
      end

      it 'selects multiple options with Array' do
        browser.select_list(name: 'new_user_languages').clear
        browser.select_list(name: 'new_user_languages').select(label: %w[EN NO])
        list = %w[EN NO]
        expect(browser.select_list(name: 'new_user_languages').selected_options.map(&:text)).to eq list
      end
    end
  ...
  describe '#select_all' do
    it 'gives deprecation notice' do
      expect {
        browser.select_list(name: 'new_user_languages').select_all('Danish')
